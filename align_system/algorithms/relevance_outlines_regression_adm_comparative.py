...
from align_system.prompt_engineering.outlines_prompts import (
    ...
    relevance_comparative_kdma_score_prediction_system_prompt,
    relevance_comparative_kdma_score_prediction_system_prompt_with_examples,
...
class OutlinesTransformersComparativeRegressionRelevanceADM(OutlinesTransformersADM):
    def __init__(self,
                 model_name,
                 device='auto',
                 baseline=False,
    ...
    def sample_kdma_score_predictions(self,
        ...
        if "number" in incontext_settings and incontext_settings["number"] > 0:
            use_icl = True
            icl_example_generator = incontext_utils.RelevanceComparativeRegressionIncontextExampleGenerator(incontext_settings,
                                                                                                            target_kdmas)
        ...
        for sample_idx in range(num_samples):
            ...
            for target_kdma in target_kdmas:
                if kdma_score_examples:
                    ...
                    kdma_score_sys_prompt = relevance_comparative_kdma_score_prediction_system_prompt_with_examples(target_kdma['name'],
                                                                                                          target_kdma['description'],
                                                                                                          score_examples,
                                                                                                          target_kdma['factor'])
                else:
                    kdma_score_sys_prompt = relevance_comparative_kdma_score_prediction_system_prompt(target_kdma['name'],
                                                                                            target_kdma['description'],
                                                                                            target_kdma['factor'])
                ...
                if use_icl:
                    ...
                    prompt_to_match = relevance_comparative_kdma_score_prediction_prompt(scenario_description,
                                                                                    no_outcome_predictions,
                                                                                    target_kdma['name'])
                ...
                predict_kdma_prompt = relevance_comparative_kdma_score_prediction_prompt(scenario_description,
        ...
        score_schema = relevance_comparative_kdma_score_prediction_json_schema(choices, target_kdma['factor'])
        ...
        for choice in choices:
            ...
            for target_kdma in target_kdmas:
                ...
                relevances[choice][target_kdma['kdma']] = []
        ...
        for sample_idx in range(num_samples):
            for kdma_idx in range(len(target_kdmas)):
                ...
                for choice in choices:
                    ...
                    relevances[choice][kdma_key].append(kdma_prediction[choice]['relevance'] / kdma_factor)

        return relevances, predictions, reasonings, icl_example_responses
    ...
    def top_level_choose_action(self,
        ...
        # Predict kdma values
        predicted_relevances, predicted_kdma_values, reasonings, icl_example_responses = self.sample_kdma_score_predictions(
        ...
        # Log predicted relevance values
        log.info("Predicted Relevance Values:")
        log.info(json.dumps(predicted_relevances))

        choice_info = {'predicted_relevances':predicted_relevances, 'true_kdma_values':true_kdma_values, 'predicted_kdma_values':predicted_kdma_values, 'icl_example_responses':icl_example_responses}
        ...
        if all_scalar_targets:
            ...
            selected_choice, probs = alignment_function(predicted_kdma_values, predicted_relevances, 
                                                        target_kdmas, probabilistic=self.probabilistic)
            best_sample_index = alignment_function.get_best_sample_index(predicted_kdma_values, predicted_relevances,
