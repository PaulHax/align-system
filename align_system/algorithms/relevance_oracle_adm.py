...
class OracleADM(ActionBasedADM):
    ...
    def choose_action(self, scenario_state, available_actions, alignment_target,
        ...
        gt_kdma_values, gt_relevance_values = {}, {}
        for action in available_actions:
            choice = action.action_id
            gt_kdma_values[choice], gt_relevance_values[choice] = {}, {}
            for target_kdma in target_kdmas:
                kdma = target_kdma['kdma']
                if hasattr(action, 'kdma_association') and action.kdma_association is not None and kdma in action.kdma_association:
                    gt_kdma_values[choice][kdma] = action.kdma_association[kdma]
                    gt_relevance_values[choice][kdma] = 1.0
                else:
                    gt_kdma_values[choice][kdma] = -1 # Placeholder (value doesn't matter because gets mutipled by 0 relevance)
                    gt_relevance_values[choice][kdma] = 0.0
        ...
        if all_scalar_targets:
            if distribution_matching == 'relevance_average':
                ...
                selected_choice_id, probs = alignment_function(
                    gt_kdma_values, gt_relevance_values, target_kdmas, misaligned=self.misaligned, probabilistic=self.probabilistic
                )
        ...
        results = pd.DataFrame([
            (action.unstructured, probs[choice])
            for action in available_actions
